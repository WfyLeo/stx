...
import com.ctx.exchange.config.IdAutoConfiguration;
...
import com.ctx.exchange.mappers.UserDtoMapper;
import com.ctx.exchange.model.*;
...
import com.ctx.exchange.mapper.UserMapper;
...
//            user.setAuthStatus(authStatus); // 认证的状态
    ...
    public Map<Long, UserDto> getBasicUsers(List<Long> ids, String userName, String mobile) {
        if (org.springframework.util.CollectionUtils.isEmpty(ids) && org.springframework.util.StringUtils.isEmpty(userName) && org.springframework.util.StringUtils.isEmpty(mobile)) {
            return Collections.emptyMap();
        }
        List<User> list = list(new LambdaQueryWrapper<User>()
                .in(!org.springframework.util.CollectionUtils.isEmpty(ids), User::getId, ids)
                .like(!org.springframework.util.StringUtils.isEmpty(userName), User::getUsername, userName)
                .like(!org.springframework.util.StringUtils.isEmpty(mobile), User::getMobile, mobile));
        if (org.springframework.util.CollectionUtils.isEmpty(list)) {
            return Collections.emptyMap();
        }
        // 将user->userDto
